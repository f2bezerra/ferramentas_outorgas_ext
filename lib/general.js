...
function waitDocumentReady(doc, options) {
	...
	var get_doc = (d) => {
		...
		d = $(d).get().find(tmp => {
			if (typeof tmp == "string" && tmp.nodeType === 1 && tmp.tagName == "IFRAME") tmp = tmp.contentDocument || tmp.contentWindow.document;
		...
		if (d instanceof Document) return d;
		if (d instanceof Window) return d.document;
		if (typeof d == "string" && d.nodeType === 1 && d.tagName == "IFRAME") return d.contentDocument || d.contentWindow.document;
		return d;
	};
