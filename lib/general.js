...
/***********************************************/
/* Extensão dos tipos nativos do javascript    */
/***********************************************/
...
if (!String.prototype.toDate) {
	String.prototype.toDate = function() {
		...
		else if (m = /^\s*(\d{2})[.-\/](\d{2})[.-\/](\d{4})\s*$/.exec(this)) return new Date(`${m[2]}/${m[1]}/${m[3]}`);
...
if (!String.prototype.toDateBR) {
	String.prototype.toDateBR = function() {
		...
		else if (m = /^\s*(\d{2})[.-\/](\d{2})[.-\/](\d{4})\s*$/.exec(this)) return this;
		return "";
	}
}
...
/*** Escrever na Área de Transferência ***
		...
		return Promise
*/
function setClipboard(text, options) {
...
function waitDocumentReady(doc, options) {
	...
	let default_options = {interval: 250,			//--- intervalo (ms) de teste do status do documento
...
function encoding(charset) {
	switch (charset) {
		case "iso-8859-1": return {	encode: (input, options) => {
...
function openDlg(dlg, options) {
	...
	let default_options = {listSeparator: ",",			// separador para listas em string
						   freeDlg: true,				// excluir dialog após fechamento
						   validate: true,				// Validar campos
						   confirmButton: undefined}; 	// identificação do botão de confirmação
...
function popupMessage(msg, title, options) {
	...
	var default_options = {title: title?title:"Mensagem", 	// título do popup
						   buttons: "OK", 					// rótulo do botão do popup
						   buttonsAlign: "center", 			// alinhamento dos botões
						   textAlign: "center"};			// alinhamento da mensagem de texto
...
function errorMessage(msg, title, options) {
	...
	var default_options = {title: title?title:"Erro", 		// título da mensagem de erro
...
function infoMessage(msg, title, options) {
	...
	var default_options = {title: title?title:"Informação",	// título da mesnagem de informação
...
function confirmMessage(msg, title, options) {
	...
	var default_options = {title: title?title:"Confirmação",	// título da mensagem de confirmação
...
function openProcessDlg(title, options) {
	...
	var default_options = {title: title?title:"Processamento",	// título do popup de processamento
	...
	var status_property = {get: function() {return $(this).attr("status")}, set: function(value) {
...
function openPopupList(target, options) {
	...
	let default_options = {list: undefined, 		// array de {text,desc,value,class}, string ou function({token, previous})
						   listSeparator: ",",		// separador para listas em string
